...
FeedbackListResponseSerializer = inline_serializer(
...
class ComplaintCreateView(EnvelopedAPIViewMixin, APIView):
    ...
    def post(self, request):
        ...
        try:
            item = ComplaintService.create_complaint(
                user=request.user,
...
class MyComplaintsView(EnvelopedAPIViewMixin, APIView):
    permission_classes = [permissions.IsAuthenticated, IsStudent]
    pagination_class = FeedbackPagination

    @extend_schema(
        tags=['Complaints'],
        summary='لیست شکایات ثبت‌شده توسط دانشجو',
        parameters=[
            OpenApiParameter(
                name='status',
    ...
    def get(self, request):
        ...
        if page is not None:
            return success_response(
                'لیست شکایات شما با موفقیت دریافت شد.',
                {
...
class ComplaintDetailView(EnvelopedAPIViewMixin, APIView):
    permission_classes = [permissions.IsAuthenticated, IsStudent]
    ...
    def get(self, request, pk):
        try:
            item = ComplaintSelector.get_complaint_for_user(request.user, pk)
        except RequestServiceError as exc:
            return error_response(exc.message, exc.errors, exc.status_code)
        ...
        return success_response('جزئیات شکایت با موفقیت دریافت شد.', data)
