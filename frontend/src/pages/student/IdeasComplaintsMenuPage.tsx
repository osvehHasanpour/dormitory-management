...
function OptionCard({ title, description, image, onClick }: OptionCardProps) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="flex w-full items-center gap-4 rounded-md border border-hairline bg-canvas p-4 text-right shadow-elevated transition-colors active:bg-surface-card"
    >
      <img
        src={image}
        alt=""
...
export function IdeasComplaintsMenuPage() {
  ...
  return (
    <div className="min-h-screen bg-surface-soft pb-32">
      ...
      <main className="mx-auto flex w-full max-w-lg flex-col px-4 sm:px-6">
        <p className="mb-6 text-center text-body-md text-ink">
