...
import type { ClassRatingPayload, ClassesTabValue, StudentClassItem } from '../types/class'
...
interface UseClassRegistrationResult {
  ...
  submitRatingForClass: (classId: number, payload: ClassRatingPayload) => Promise<boolean>
...
export function useClassRegistration(): UseClassRegistrationResult {
  ...
  const [classesByTab, setClassesByTab] = useState<ClassesByTab>(INITIAL_CLASSES)
  const [loadingByTab, setLoadingByTab] = useState<LoadingByTab>(INITIAL_LOADING)
  ...
  const loadTabData = useCallback(
    async (tab: ClassesTabValue, force = false) => {
      ...
      try {
        ...
        setClassesByTab((previous) => ({ ...previous, [tab]: data.results ?? [] }))
        setLoadedByTab((previous) => ({ ...previous, [tab]: true }))
      } catch (loadError) {
    ...
    async (classId: number): Promise<boolean> =>
      runAction(
        ...
        async () => {
          ...
          await Promise.all([loadTabData('active', true), loadTabData('enrolled', true)])
        },
  ...
  const classes = useMemo(() => classesByTab[activeTab], [activeTab, classesByTab])
