import type { FeedbackAuthorSummary, FeedbackStatus, SupervisorFeedbackItem } from '../types/supervisorFeedback'
...
export function getFeedbackStatusBadgeVariant(status: FeedbackStatus): StatusBadgeVariant {
...
export function buildFeedbackPreview(description: string, maxLength = 100): string {
...
export function canMarkReviewed(item: SupervisorFeedbackItem): boolean {
  return (
    (item.type === 'complaint' || item.type === 'suggestion') && item.status === 'pending'
  )
}
...
export function hasAvailableFeedbackActions(item: SupervisorFeedbackItem): boolean {
  if (item.type === 'idea') {
